...
- **推荐公司 + 投递追踪**：AI 推荐适合的目标公司，支持标记"已投递"并持久化
- **里程碑路线图**：可视化的横向闯关路线图，涵盖从入学到毕业的各个阶段

### 3. 打卡与宝箱系统
- **路标打卡**：点击路标节点记录经历，获得金币奖励
- **宝箱玩法**：每个已完成路标可开启宝箱，AI 生成专属点评反馈
- **钻石收藏**：已开启的宝箱变为 💎 钻石，点击即可查看历史点评，无需重复生成

### 4. 简历信息收集
- 教育背景、专业技能、语言能力、证书
- 实习经历、竞赛经历、学术成果
- 荣誉奖项、社团经历（结构化条目，支持添加多条）
- 数据自动同步到简历预览和求职广场
...
### 8. 游戏化体验
- **超级马里奥风格界面**：云朵、金币、水管、问号砖块等经典元素
- **金币收集**：完成里程碑获得金币，点击背景金币收集
- **成就系统**：记录用户的职业规划进度
- **小恐龙跑酷**：AI 生成时的趣味等待小游戏，支持踩蘑菇得积分
- **6 种精美主题**：默认/森林/海洋/夜间/樱花/橙色，一键切换

### 9. 状态持久化
- 表单数据、标签选择、AI 规划自动保存到服务端
- 刷新页面恢复上次活跃的 Tab 和求职广场搜索结果
- 退出登录后重新登录回到首页
...
或双击 `start.bat`（Windows 专用，自动使用 conda 环境）。
...
│   ├── start.bat           # Windows 启动脚本（careerPlan 环境）
│   ├── quick-start.bat     # 一键重启（自动杀旧进程+重启+装依赖）
│   ├── templates/          # HTML 模板（如果有）
│   ├── uploads/            # 上传文件存储
│   └── career_planner.db   # SQLite 数据库（运行后自动生成）
├── logo-designs/           # 应用图标
├── career-planner.html     # 前端主页面（单页应用）
└── README.md
```
...
| `/api/ai/generate` | POST | AI 生成职业规划 | ✅ |
